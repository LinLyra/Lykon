...
bool qmi8658_Read_AccAndGry(t_sQMI8658 *p)
{
    uint8_t status = 0;
    uint8_t raw[12] = {0};

    esp_err_t ret = qmi8658_register_read(QMI8658_STATUS0, &status, 1);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "QMI8658 read STATUS0 failed: %s", esp_err_to_name(ret));
        return false;
    }

    // bit0/bit1 表示加速度/陀螺仪数据 ready
    if ((status & 0x03) == 0) {
        return false;
    }

    ret = qmi8658_register_read(QMI8658_AX_L, raw, 12);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "QMI8658 read raw data failed: %s", esp_err_to_name(ret));
        return false;
    }
    ...
    // 32767 全通道基本是无效数据，直接丢弃
    if (acc_x == 32767 && acc_y == 32767 && acc_z == 32767 &&
        gyr_x == 32767 && gyr_y == 32767 && gyr_z == 32767) {
        ESP_LOGW(TAG,
                 "QMI8658 invalid raw data: status=0x%02x raw=%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x",
                 status,
...
/***************    LCD显示屏 ↑   *************************/
/***********************************************************/
void lcd_fill_rect(int x_start, int y_start, int x_end, int y_end, uint16_t color)
{
    ...
    if (buffer == NULL) {
        ESP_LOGE(TAG, "Memory for rect is not enough");
        return;
    }
    ...
    for (int y = y_start; y < y_end; y++) {
        esp_lcd_panel_draw_bitmap(panel_handle, x_start, y, x_end, y + 1, buffer);
    }
